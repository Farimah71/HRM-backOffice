...
import {
  createCandidatePositionOffer,
  editCandidatePositionOffer,
  getByIdCandidatePositionOffer,
  getCandidatePositionOffer,
} from "../../../../../redux/actions/candidate-position-offer/index";
...
export const PositionOffer = ({ onCloseModal, candidatePositionId }) => {
  // ---------- store ----------
  const {
    info: candidateOfferData,
    loading,
    editInfo,
  } = useSelector((state) => state.candidatePositionOfferSlice);
  ...
  const cols = [
    ...
    {
      name: t("table.col.candidate"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.candidateName}</p>
      ),
    },
    {
      name: t("table.col.offer_content"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.offerContent}</p>
      ),
    },
    {
      name: t("table.col.result"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.offerResult}</p>
      ),
    },
    {
        ...
        <div className="group relative flex items-center justify-center gap-x-2">
          <span
            ...
            onClick={() => {
              ...
              dispatch(getByIdCandidatePositionOffer(row.id));
            }}
          >
            <svg
              width="17"
  ...
  const dataSchema = Yup.object({
    offerDate: Yup.string().required(t("error.offer_date_required")),
  });
  ...
  const deleteHandler = (id) => {
    axios
      .delete(api.CandidatePositionOfferApi.deleteCandidatePositionOffer + id, {
        headers,
      })
      .then((res) => {
  ...
  const reloadPageHandler = (status) => {
    ...
    dispatch(
      getCandidatePositionOffer({
        ...
        orderBy: "",
        includeProperties:
          "CandidatePosition,CandidatePosition.Candidate,CandidatePosition.CompanyPosition",
      })
    );
  };
  const closeHandler = () => {
  ...
  const onSubmit = (values) => {
    ...
    const dataToSend = {
      ...clone,
      offerDate: clone.offerDate
        ? convertDateToString(clone.offerDate)
        : editInfo.offerDate,
    };
    isShowForm.create &&
      dispatch(
        createCandidatePositionOffer(dataToSend, (status) =>
          reloadPageHandler(status)
        )
      );
    isShowForm.edit &&
      dispatch(
        editCandidatePositionOffer(values.id, dataToSend, (status) =>
          reloadPageHandler(status)
        )
      );
  };
  ...
  useEffect(() => {
    if (candidateOfferData.length) {
      const values = candidateOfferData.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.candidatePosition.companyPosition.title,
        candidateName:
          item.candidatePosition.candidate.name +
          " " +
          item.candidatePosition.candidate.family,
  ...
  useEffect(() => {
    setFormValue({
      ...
      offerDate: editInfo.offerDate && new Date(editInfo.offerDate),
      offerContent: editInfo.offerContent,
      offerResult: editInfo.offerResult,
    });
  }, [editInfo]);
  ...
  return (
    <>
      <div className="flex items-center justify-between p-4 rounded-t-lg select-none border-b border-custom-gray-light dark:border-0 dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">
          {t("page_title.position_offer")}
        </h4>
        <div
      ...
      <div className="ml-auto mr-3 m-5">
        <Button
      ...
      <div className="flex-grow flex flex-col overflow-y-auto p-3 mb-10">
        {isLoading ? (
        ...
        {(isShowForm.create || isShowForm.edit) && (
          ...
          >
            <p className="font-semibold mb-5 dark:text-dark_custom-full-white">
              {isShowForm.create
                ? t("page_title.new_position_offer")
                : t("page_title.edit_position_offer")}
            </p>
            <hr />
            <div className="w-full flex gap-4 my-3">
              <div className="w-1/3">
                <Field
                  ...
                  placeholder={t("input.offer_date.placeholder")}
                  label={t("input.offer_date.label")}
                  name="offerDate"
                />
                <Error name="offerDate" />
              </div>
              <div className="w-1/3">
                <Field
                  component={Input}
                  placeholder={t("input.offer_content.placeholder")}
                  label={t("input.offer_content.label")}
                  name="offerContent"
                />
              </div>
              <div className="w-1/3">
                <Field
                  component={Input}
                  placeholder={t("input.offer_result.placeholder")}
                  label={t("input.offer_result.label")}
                  name="offerResult"
                />
              </div>
              <div className="w-1/10 flex gap-x-1 mt-8">
                <Field
                  component={Button}
