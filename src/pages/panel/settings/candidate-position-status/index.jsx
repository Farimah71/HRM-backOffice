...
export const CandidatePositionStatus = () => {
  ...
  // ---------- store ----------
  const { info: candidatePositionStatusData, loading } = useSelector(
    (state) => state.candidatePositionStatusSlice
  );
  ...
  console.log(candidatePositionStatusData.data);

  let candidatePositionStatusDataNew=useMemo(() => {
    return candidatePositionStatusData.data && candidatePositionStatusData.data.map((candidatePositionStatus) => {
      return {
        ...candidatePositionStatus,
        inverseParent:candidatePositionStatusData.data.filter((candidate) => {
          return candidate.parentId==candidatePositionStatus.id
        })
      }
    })
  } , [candidatePositionStatusData.data]);

  console.log(candidatePositionStatusDataNew);

  console.log("new" , candidatePositionStatusDataNew);



  function renderInverseParent(inverseParent) {


    let mainInverseParent=inverseParent && inverseParent.map((candidatePositionStatus) => {
      return {
        ...candidatePositionStatus,
        inverseParent:candidatePositionStatusData.data.filter((candidate) => {
          return candidate.parentId==candidatePositionStatus.id
    ...
    return mainInverseParent.map(
      (subCandidate) => {
        return (
          <li className="w-full dark:text-dark_custom-full-white" key={subCandidate.id}>
            <div className="flex items-center justify-between">
              <div onClick={() => setShow({...show , [subCandidate.id]:!show[subCandidate.id]})}
            ...
            {!!subCandidate.inverseParent.length && (
                            <div
                              className={`ml-8 ${
                                show && show[subCandidate.id] ? "block mt-5" : "hidden"
                              }`}
                            >
                              <ul className="flex flex-col gap-y-6">
                                {
                                  renderInverseParent(subCandidate.inverseParent)
                                }
                              </ul>
                            </div>
                          )}
  ...
  // useEffect(() => {
  //   let idKeyAndValue=candidatePositionStatusData.data && candidatePositionStatusData.data.map((candidatePositionStatus) => {
  //     return {
  //       [candidatePositionStatus.id]:false
  //     }
  //   })


  //   setShow(idKeyAndValue);
          ...
          {isLoading && !candidatePositionStatusData.count ? (
            ...
            <div>
              <ul className="flex flex-col gap-y-5">
                {candidatePositionStatusData.count ?
                  candidatePositionStatusDataNew.map(
                    (candidatePosition, index) =>
                      !candidatePosition.parentId && (
                        <li className="w-2/6" key={candidatePosition.id}>
                          <div className="flex items-center justify-between">
                            <div
                              className="flex items-center gap-x-2 cursor-pointer dark:text-dark_custom-full-white"
                              onClick={() => setShow({...show , [candidatePosition.id]:!show[candidatePosition.id]})}
                            >
                          ...
                          {!candidatePosition.parentId && (
                            <div
                              className={`ml-8 ${
                                show && show[candidatePosition.id] ? "block mt-5" : "hidden"
                              }`}
                            >
                              <ul className="flex flex-col gap-y-6">
                                {
                                  renderInverseParent(candidatePosition.inverseParent)
                                }
                              </ul>
                            </div>
                          )}
