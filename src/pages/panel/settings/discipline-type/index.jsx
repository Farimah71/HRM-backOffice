...
import { CreateDisciplineType } from "./create";
import { EditDisciplineType } from "./edit";

export const DisciplineType = () => {
  // ---------- store ----------
  const { info: disciplineTypeData, loading } = useSelector(
    (state) => state.disciplineTypeSlice
  );
  ...
  useEffect(() => {
    if (disciplineTypeData) {
      const values = disciplineTypeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    axios
      .delete(api.SettingsApi.deleteDisciplineType + id, { headers })
      .then((res) => {
