...
import {
  editDisciplineType,
  getByIdDisciplineType,
} from "../../../../../redux/actions/settings/discipline-type";
import { Field } from "formik";
import * as Yup from "yup";

export const EditDisciplineType = ({
  onCloseModal,
  isReloadPage,
  editId,
  loading,
  ...
  // ---------- store ----------
  const editInfo = useSelector((state) => state.disciplineTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editDisciplineType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdDisciplineType(editId));
  }, []);
  useEffect(() => {
    ...
    <>
      <div className="flex items-center justify-between p-4 border-b border-custom-gray-light dark:bg-dark_custom-average-black">
        <h4 className="text-18 font-bold dark:text-dark_custom-full-white">
          {t("page_title.edit_disciplineType")}
        </h4>
        <div
