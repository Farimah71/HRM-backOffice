...
import {
  editFileType,
  getByIdFileType,
} from "../../../../../redux/actions/settings/file-type";
import { Field } from "formik";
import * as Yup from "yup";

export const EditFileType = ({
  onCloseModal,
  isReloadPage,
  editId,
  loading,
  ...
  // ---------- store ----------
  const editInfo = useSelector((state) => state.fileTypeSlice.editInfo);
  ...
  const onSubmit = (values) => {
    dispatch(
      editFileType(editId, values, (status) => reloadPageHandler(status))
    );
  };

  // ---------- lifeCycle ----------
  useEffect(() => {
    dispatch(getByIdFileType(editId));
  }, []);
  useEffect(() => {
