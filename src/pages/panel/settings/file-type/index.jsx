...
import { api } from "../../../../api";
import { getFileTypes } from "../../../../redux/actions/settings/file-type";
import { CreateFileType } from "./create";
import { EditFileType } from "./edit";

export const FileType = () => {
  // ---------- store ----------
  const { info: fileTypeData, loading } = useSelector(
  ...
  useEffect(() => {
    if (fileTypeData) {
      const values = fileTypeData.data?.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
      }));
  ...
  const deleteHandler = (id) => {
    axios
      .delete(api.SettingsApi.deleteFileType + id, { headers })
      .then((res) => {
