...
import {
  createEmployeeLeave,
  editEmployeeLeave,
  getByIdEmployeeLeave,
  getEmployeeLeave,
} from "../../../../../redux/actions/employee-leave";
import {
  errorNotification,
  successNotification,
...
export const Leave = ({ state, onCloseModal }) => {
  // ----------store-----------
  const {
    info: employeeLeaveData,
    count,
    editInfo,
    loading,
  } = useSelector((state) => state.employeeLeaveSlice);
  const { info: leaveTypeData, loading: leaveTypeLoading } = useSelector(
    (state) => state.leaveTypeSlice
  );
  const { info: permissionTypeData, loading: permissionTypeLoading } =
    useSelector((state) => state.permissionTypeSlice);
  ...
  useEffect(() => {
    dispatch(
      getEmployeeLeave({
        ...
        includeProperties:
          "Employee.CandidatePosition.Candidate,LeaveType,PermissionType",
      })
    );
  ...
  useEffect(() => {
    if (employeeLeaveData.count && employeeLeaveData?.data.length > 0) {
      const values = employeeLeaveData.data.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        ...
        permissionType: item.permissionType.title,
  ...
  useEffect(() => {
    setFormValue({
      ...
      endDate: editInfo.endDate && new Date(editInfo.endDate),
      permissionTypeId: editInfo.permissionTypeId,
      leaveTypeId: editInfo.leaveTypeId,
      description: editInfo.description,
    });
    setSelectedOption({
      permissionType: {
        value: editInfo.permissionType?.id,
        label: editInfo.permissionType?.title,
      },
      leaveType: {
        value: editInfo.leaveType?.id,
        label: editInfo.leaveType?.title,
      },
    });
  }, [editInfo]);
  useEffect(() => {
    if (permissionTypeData.count && permissionTypeData?.data.length > 0) {
      const option = convertArrayToSelectOptions(permissionTypeData.data, [
        "id",
        "title",
      ]);
      setSelectOption((prev) => ({ ...prev, permissionType: option }));
    }
  }, [permissionTypeData]);
  useEffect(() => {
    if (leaveTypeData.count && leaveTypeData?.data.length > 0) {
      const option = convertArrayToSelectOptions(leaveTypeData.data, [
        "id",
        "title",
      ]);
      setSelectOption((prev) => ({ ...prev, leaveType: option }));
    }
  }, [leaveTypeData]);
  ...
  const cols = [
    ...
    {
      name: t("table.col.permission_type"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.permissionType}</p>
      ),
    },
    {
      name: t("table.col.leave_type"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.leaveType}</p>
      ),
    },
    {
      name: t("table.col.start"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.startDate}</p>
      ),
    },
    {
      name: t("table.col.end"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.endDate}</p>
      ),
    },
    {
        ...
        <div className="flex gap-x-1">
          <span
            ...
            onClick={() => {
              ...
              dispatch(
                getByIdEmployeeLeave(row.id, "PermissionType,LeaveType")
              );
            }}
          >
            <svg
  ...
  const dataSchema = Yup.object({
    leaveTypeId: Yup.number().required(t("error.leave_type_required")),
    permissionTypeId: Yup.number().required(
      t("error.permission_type_required")
    ),
    startDate: Yup.string().required(t("error.start_date_required")),
    endDate: Yup.string().required(t("error.end_date_required")),
  });
  const headers = {
  ...
  const deleteHandler = (id) => {
    axios
      .delete(api.EmployeeLeaveApi.deleteEmployeeLeave + id, {
        headers,
      })
      .then((res) => {
  ...
  const onSubmit = (values) => {
    ...
    const dataToSend = {
      ...
      startDate: clone.startDate
        ? convertDateToString(clone.startDate)
        : editInfo.startDate,
      endDate: clone.endDate
        ? convertDateToString(clone.endDate)
        : editInfo.endDate,
    };
    isShowForm.create &&
      dispatch(
        createEmployeeLeave(dataToSend, (status) => reloadPageHandler(status))
      );
    isShowForm.edit &&
      dispatch(
        editEmployeeLeave(values.id, dataToSend, (status) =>
        ...
        {(isShowForm.create || isShowForm.edit) && (
          ...
          >
            <p className="font-semibold mb-5 dark:text-dark_custom-full-white">
              {isShowForm.create
                ? t("page_title.new_employee_leave")
                : t("page_title.edit_employee_leave")}
            </p>
            <hr />
            <div className="w-full flex flex-col gap-4 my-3">
              <div className="flex gap-x-2">
                <div className="w-1/3">
                  <Field
                    component={SelectBox}
                    placeholder={t("input.leave_type.placeholder")}
                    label={t("input.leave_type.label")}
                    name="leaveTypeId"
                    options={selectOption.leaveType}
                    selectedOption={selectedOption.leaveType}
                    loading={leaveTypeLoading}
                  />
                  <Error name="leaveTypeId" />
                </div>
                ...
                <div className="w-1/3">
                  <Field
                    component={CustomDatePicker}
                    placeholder={t("input.end_date.placeholder")}
                    label={t("input.end_date.label")}
                    name="endDate"
                  />
                  <Error name="endDate" />
                </div>
              ...
              <div className="flex gap-x-2">
                <div className="w-1/3">
                  <Field
                    component={SelectBox}
                    placeholder={t("input.permission_type.placeholder")}
                    label={t("input.permission_type.label")}
                    name="permissionTypeId"
                    options={selectOption.permissionType}
                    selectedOption={selectedOption.permissionType}
                    loading={permissionTypeLoading}
                  />
                  <Error name="permissionTypeId" />
                </div>
                ...
                <div className="w-1/10 flex gap-x-1 mt-20 mr-2 ml-auto">
                  <Field
                    component={Button}
