...
import { CreateDiscipline } from "./form/create";
import { EditDiscipline } from "./form/edit";
import {
  errorNotification,
  successNotification,
} from "../../../../../helpers/notification";
import {
  getByIdEmployeeDiscipline,
  getEmployeeDisciplines,
} from "../../../../../redux/actions/employee-discipline";
import { api } from "../../../../../api";
import axios from "axios";

export const EmployeeDiscipline = ({ state, onCloseModal }) => {
  // ----------store-----------
  const {
    info: employeeDisciplineData,
    count,
    editInfo,
    loading,
  } = useSelector((state) => state.employeeDisciplineSlice);
  ...
  useEffect(() => {
    dispatch(
      getEmployeeDisciplines({
        ...
        includeProperties: "DisciplineType",
      })
    );
  }, []);
  useEffect(() => {
    if (employeeDisciplineData.length > 0) {
      const values = employeeDisciplineData.map((item, index) => ({
        rowId: index + 1,
        id: item.id,
        title: item.title,
        issueDate: item.issueDate.split("T")[0],
        disciplineType: item.disciplineType?.title,
  ...
  const cols = [
    ...
    {
      name: t("table.col.discipline_type"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.disciplineType}</p>
      ),
    },
    {
      name: t("table.col.issue_date"),
      selector: (row) => (
        <p className="dark:text-dark_custom-full-white">{row.issueDate}</p>
      ),
    },
    {
        ...
        <div className="flex gap-x-1">
          <span
            ...
            onClick={() => {
              ...
              dispatch(getByIdEmployeeDiscipline(row.id, "DisciplineType"));
            }}
          >
            <svg
              width="15"
  ...
  const deleteHandler = (id) => {
    axios
      .delete(api.EmployeeDisciplineApi.deleteEmployeeDiscipline + id, {
        headers,
      })
      .then((res) => {
        ...
        <div className="items-center dark:bg-dark_custom-light-black">
          <div className="flex justify-between gap-y-1 mb-5">
            <h4 className="text-19 text-custom-dark font-bold dark:text-dark_custom-full-white">
              {t("page_title.employee_discipline")}
            </h4>
            <div
